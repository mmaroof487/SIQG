...
The core flow involves a **4-Layer Pipeline** executed on every query request:
1. **Security Layer:** Identity, authorization, and threat prevention.
2. **Performance Layer:** Caching, limits, budgeting, and cost analysis.
3. **Execution Layer:** Abstracted routing, resilience, and timeout management.
4. **Observability Layer:** Metrics, audits, and intelligent anomaly detection.
...
- **Logic:** Blocklist takes precedence. If an allowlist exists, the IP *must* be in it.
...
- **State Machine:** Maintained in Redis.
  ...
  - *OPEN:* Error threshold exceeded; all requests fast-fail with 503 Service Unavailable.
  - *HALF-OPEN:* After a cooldown period, the next request acts as a single probe. If successful, closes the circuit. If it fails, re-opens it.
